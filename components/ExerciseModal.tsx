...
const ExerciseModal: React.FC<ExerciseModalProps> = ({ onClose, onSave, initialExercise, isEditing }) => {
  const [editingExercise, setEditingExercise] = useState<Partial<Exercicio>>(initialExercise || {});
      ...
      <div className="bg-[#111] border border-[#333] p-6 sm:p-8 rounded-[2.5rem] sm:rounded-[3rem] w-full max-w-2xl shadow-2xl animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center mb-8 shrink-0">
          <div>
            ...
            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Personalize sua biblioteca</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 overflow-y-auto no-scrollbar pr-4 flex-1">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-[#1A1A1A] p-4 rounded-2xl border border-[#222]">
              <label className="text-[8px] font-black text-gold uppercase mb-2 block">Nome do Exercício</label>
              <input 
                type="text" 
            ...
            <div className="bg-[#1A1A1A] p-4 rounded-2xl border border-[#222]">
              <label className="text-[8px] font-black text-gold uppercase mb-2 block">Grupo Muscular</label>
              <select 
                className="bg-transparent text-white font-bold outline-none w-full"
                ...
                onChange={e => {
                  ...
                  setEditingExercise({...editingExercise, grupoId: group, grupoNome: groupName, subgrupoId: group, subgrupoNome: groupName});
                }}
              >
          ...
          <div className="bg-[#1A1A1A] p-4 rounded-2xl border border-[#222]">
            <label className="text-[8px] font-black text-gold uppercase mb-2 block flex items-center">
              <Youtube size={12} className="mr-2" /> URL do Vídeo (YouTube/Instagram)
            </label>
            <input 
              type="text" 
            ...
            />
            <p className="text-[7px] text-gray-600 mt-2 uppercase font-bold">Dica: Use vídeos do seu canal UCL8yaItGFmCTPNFCd-Q4_BQ</p>
          </div>

          <div className="bg-[#1A1A1A] p-4 rounded-2xl border border-[#222]">
            <label className="text-[8px] font-black text-gold uppercase mb-2 block">Descrição / Dicas Técnicas</label>
            <textarea 
              className="bg-transparent text-white font-bold outline-none w-full h-24 resize-none" 
              value={editingExercise.descricao || ''} 
              onChange={e => setEditingExercise({...editingExercise, descricao: e.target.value})}
            />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[#1A1A1A] p-4 rounded-2xl border border-[#222]">
              <label className="text-[8px] font-black text-gold uppercase mb-2 block">Equipamento</label>
              <input 
                type="text" 
                className="bg-transparent text-white font-bold outline-none w-full" 
                value={editingExercise.equipamento || ''} 
                onChange={e => setEditingExercise({...editingExercise, equipamento: e.target.value})} 
              />
            </div>
            <div className="bg-[#1A1A1A] p-4 rounded-2xl border border-[#222]">
              <label className="text-[8px] font-black text-gold uppercase mb-2 block">Nível</label>
              <select 
                className="bg-transparent text-white font-bold outline-none w-full"
                value={editingExercise.nivel || 'Iniciante'}
                onChange={e => setEditingExercise({...editingExercise, nivel: e.target.value})}
              >
        ...
        <div className="flex gap-4 pt-8 shrink-0">
          <button type="button" onClick={onClose} className="flex-1 py-5 text-gray-500 font-black uppercase text-xs tracking-widest hover:text-white transition-colors">Cancelar</button>
          <button 
