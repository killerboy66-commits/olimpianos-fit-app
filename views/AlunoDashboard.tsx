...
import { Usuario, Route, Treino, Periodizacao, Conquista, CronogramaSemanal, RegistroProgresso, Atribuicao } from '../types';
import { db } from '../services/storage';
import { CONQUISTAS_DATA } from '../data/conquistas';
import * as Icons from 'lucide-react';
import { Play, Target, Clock, ChevronRight, ChevronLeft, ListChecks, Activity, Trophy, Zap, CalendarRange, CalendarDays, CheckCircle2, Circle, ExternalLink, ShieldCheck, Flame, Sun, Star, X, Info, Sword, Shield, Bird, Mountain, Gem } from 'lucide-react';
...
const AlunoDashboard: React.FC<AlunoDashboardProps> = ({ user, onStartWorkout, onNavigate, onLogout }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setSchedule(s);
        setProgress(p.filter(item => item.user_id === user.id));
        setTreinos(t);
        setPeriodizacao(per);
        setAssignments(a.filter(item => item.user_id === user.id && item.ativo));
      } catch (error) {
  ...
  const date = new Date();
  const dayIndex = date.getDay(); // 0 (Dom) a 6 (Sab)
  const adjustedDayIndex = dayIndex === 0 ? 6 : dayIndex - 1; // Ajusta para 0 (Seg) a 6 (Dom)
  ...
  const workoutToday = workoutIdToday && workoutIdToday !== 'Descanso' ? treinos.find(t => t.id === workoutIdToday) : null;

  // Filtrar fichas atribuídas pelo professor
  const activeWorkouts = treinos.filter(t => assignments.some(a => a.workout_id === t.id));
  ...
  // Lógica para verificar treinos realizados na semana atual
  ...
  // Cálculo da Fase Atual da Periodização
  const getCurrentPhase = () => {
    if (!periodizacao || assignments.length === 0) return null;
    
    // Usar a data da primeira atribuição ativa como início do ciclo do aluno
    const firstAssignment = assignments.reduce((prev, curr) => 
  ...
  const getRank = (workoutCount: number) => {
    if (workoutCount >= 100) return { title: 'Soberano do Olimpo', color: 'text-gold', icon: <Zap size={10} />, bg: 'bg-gold/10' };
    if (workoutCount >= 50) return { title: 'Semideus Imortal', color: 'text-amber-400', icon: <Zap size={10} />, bg: 'bg-amber-400/10' };
    if (workoutCount >= 25) return { title: 'Herói Lendário', color: 'text-emerald-400', icon: <Trophy size={10} />, bg: 'bg-emerald-400/10' };
    if (workoutCount >= 10) return { title: 'Guerreiro de Elite', color: 'text-blue-400', icon: <Shield size={10} />, bg: 'bg-blue-400/10' };
    return { title: 'Recruta Espartano', color: 'text-gray-400', icon: <Sword size={10} />, bg: 'bg-gray-400/10' };
  };
    ...
    <div className="space-y-6 sm:space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <header className="flex items-center justify-between px-1">
        <div className="flex items-center gap-3">
          <div>
            <h2 className="text-2xl sm:text-3xl font-black italic uppercase tracking-tighter text-white">Santuário</h2>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:space-x-2">
              <p className="text-gray-500 text-[11px] sm:text-sm font-medium">Bora, <span className="text-gold font-bold">{user.nome.split(' ')[0]}</span>!</p>
              <div className={`flex items-center space-x-1 px-2 py-0.5 rounded-full border border-current w-fit ${rank.color} ${rank.bg}`}>
                {rank.icon}
                <span className="text-[7px] sm:text-[8px] font-black uppercase tracking-widest">{rank.title}</span>
              </div>
            </div>
          </div>
          
          {/* Botão de Sair Mobile - Próximo ao Santuário */}
          <button 
            onClick={onLogout}
        ...
        >
          ...
          <img src={getAvatar(user.foto)} className="w-full h-full rounded-lg sm:rounded-xl bg-[#111] object-cover group-hover:brightness-125 transition-all relative z-10" alt="Perfil" />
        </button>
      </header>
          ...
          <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex flex-col items-center text-center space-y-6 relative z-10">
                ...
                <div>
                  <h3 className="text-3xl font-black italic uppercase tracking-tighter text-white mb-1">{user.nome}</h3>
                  <div className={`inline-flex items-center space-x-2 px-4 py-1.5 rounded-full border border-current ${rank.color} ${rank.bg}`}>
                    {rank.icon}
                    <span className="text-[10px] font-black uppercase tracking-widest">{rank.title}</span>
                  </div>
                </div>

                <div className="w-full grid grid-cols-2 gap-4">
                  <div className="bg-white/5 border border-white/5 p-4 rounded-2xl text-left">
                    <p className="text-[8px] font-black text-gold uppercase tracking-widest mb-1">E-mail</p>
                    <p className="text-xs font-bold text-gray-300 truncate">{user.email}</p>
                  </div>
                  <div className="bg-white/5 border border-white/5 p-4 rounded-2xl text-left">
                    <p className="text-[8px] font-black text-gold uppercase tracking-widest mb-1">Total Treinos</p>
                ...
                <div className="w-full bg-white/5 border border-white/5 p-5 rounded-3xl text-left">
                  <div className="flex items-center space-x-2 mb-3">
                    ...
                    <p className="text-[9px] font-black text-gold uppercase tracking-widest">Objetivo Atual</p>
                  </div>
           ...
           <div className="bg-[#111] border border-[#222] p-4 sm:p-6 rounded-[2rem] sm:rounded-[2.5rem] relative overflow-hidden">
             ...
             <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 sm:mb-6">
               <div className="flex items-center space-x-3 sm:space-x-4">
                 ...
                 <div>
                   <p className="text-[8px] sm:text-[10px] font-black text-gray-500 uppercase tracking-widest">Mês Vigente</p>
                   <h4 className="text-lg sm:text-xl font-black text-white uppercase italic">{currentPhase.fase}</h4>
                 </div>
               </div>
               <div className="flex justify-between sm:justify-end gap-4 sm:space-x-3 bg-black/20 p-2 rounded-xl sm:bg-transparent sm:p-0">
                 <div className="text-center">
                   <p className="text-[7px] font-black text-gray-600 uppercase mb-1">Intensidade</p>
                   <div className="flex space-x-0.5">
                     {[...Array(10)].map((_, i) => (
                       <div key={i} className={`w-1 h-3 rounded-full ${i < (currentPhase.intensidade || 5) ? 'bg-gold' : 'bg-white/5'}`}></div>
                     ))}
                   </div>
                 </div>
                 <div className="text-center">
                   ...
                   <div className="flex space-x-0.5">
                     {[...Array(10)].map((_, i) => (
                       <div key={i} className={`w-1 h-3 rounded-full ${i < (currentPhase.volume || 5) ? 'bg-emerald-500' : 'bg-white/5'}`}></div>
                     ))}
                   </div>
                 </div>
               </div>
      ...
      <section className="animate-in slide-in-from-right-4 duration-500 delay-200">
        ...
        >
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h4 className="font-black text-sm uppercase italic text-white">Acessar Site Oficial</h4>
              <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Conteúdos exclusivos e novidades</p>
            </div>
          </div>
      ...
      {/* Card de Ação Principal: Agenda Semanal e Abrir Sessão */}
      <section>
        <div className="bg-gradient-to-br from-[#1A1A1A] to-[#0A0A0A] border-2 border-gold/20 p-5 sm:p-8 rounded-[2.5rem] sm:rounded-[3rem] relative overflow-hidden shadow-2xl">
           ...
           <div className="relative z-10">
             <div className="flex items-center justify-between mb-6 sm:mb-8">
               <div>
                 <p className="text-gray-500 text-[8px] sm:text-[10px] font-black uppercase tracking-[0.2em] mb-1">Agenda Semanal</p>
                 <h4 className="text-xl sm:text-2xl font-black text-white uppercase italic">Sessão de Hoje</h4>
               </div>
               ...
               {DIAS_KEYS.map((key, idx) => {
                 ...
                 return (
                   <div key={key} className="flex flex-col">
                    ...
                    >
                      <span className={`text-[7px] sm:text-[8px] font-black uppercase mb-0.5 sm:mb-1 ${isToday ? 'text-gold' : isDone ? 'text-emerald-400' : 'text-gray-500'}`}>
                        {DIAS_LABELS[idx]}
                      </span>
             ...
             <button 
               onClick={() => {
                 onStartWorkout(workoutToday?.id || activeWorkouts[0]?.id || 'A');
               }}
               className="w-full bg-gold text-[#0A0A0A] py-4 sm:py-5 rounded-2xl sm:rounded-[2rem] font-black text-base sm:text-xl flex items-center justify-center space-x-2 sm:space-x-3 hover:brightness-110 transition-all shadow-xl active:scale-[0.98]"
             >
               <Play fill="currentColor" size={20} />
               <span className="uppercase tracking-widest">Iniciar Treino</span>
             </button>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="text-left">
              <div className="flex items-center space-x-2">
                <span className="bg-gold text-black text-[8px] font-black px-1.5 py-0.5 rounded uppercase">Ativo</span>
                <p className="font-black text-lg text-white">Desafio GymRats</p>
              </div>
              <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-0.5">Veja sua posição no ranking mundial</p>
            </div>
          </div>
      ...
      <section className="animate-in slide-in-from-left-4 duration-500 delay-300">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-black uppercase tracking-[0.2em] flex items-center">
            ...
            <span className="text-gold font-black text-xs sm:text-sm">Conquistas Desbloqueadas</span>
          </h3>
          <div className="flex items-center space-x-2">
            ...
            <span className="text-[10px] font-black text-gold uppercase">{userConquistas.length} / {CONQUISTAS_DATA.length}</span>
          </div>
        </div>
            ...
            {CONQUISTAS_DATA.map((conquista) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <Icon size={32} strokeWidth={isEarned ? 2.5 : 2} style={isEarned ? { filter: `drop-shadow(0 0 12px ${conquista.cor})` } : {}} />
                  </div>
                  <p className={`text-[10px] font-black uppercase tracking-tighter leading-tight mb-2 ${isEarned ? 'text-white' : 'text-gray-400'}`}>
                    {conquista.titulo}
                  </p>
                  <div className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-full inline-block shadow-sm ${styles.badge}`}>
                    {conquista.raridade}
                  </div>
                </button>
              );
              ...
              <div className="flex flex-col items-center text-center">
                ...
                >
                  {React.createElement((Icons as any)[selectedConquista.icone] || Trophy, { 
                    ...
                    style: user.conquistas_ids?.includes(selectedConquista.id) ? { filter: `drop-shadow(0 0 15px ${selectedConquista.cor})` } : {}
                  })}
                </div>

                <div className={`text-[11px] font-black uppercase px-4 py-1.5 rounded-full mb-4 shadow-lg ${
                  selectedConquista.raridade === 'lendario' ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white' :
                  selectedConquista.raridade === 'epico' ? 'bg-gradient-to-r from-blue-600 to-cyan-600 text-white' :
                  selectedConquista.raridade === 'raro' ? 'bg-gradient-to-r from-gold to-amber-500 text-black' : 
                  'bg-gradient-to-r from-gray-600 to-gray-500 text-white'
                }`}>
                  {selectedConquista.raridade}
                </div>

                <h4 className="text-3xl font-black text-white uppercase italic mb-3 tracking-tighter">{selectedConquista.titulo}</h4>
                ...
                {user.conquistas_ids?.includes(selectedConquista.id) ? (
                  <div className="flex items-center space-x-3 text-emerald-400 bg-emerald-500/10 px-6 py-3 rounded-2xl border border-emerald-500/20 shadow-lg shadow-emerald-500/5">
                    ...
                    <span className="text-xs font-black uppercase tracking-widest">Conquista Desbloqueada</span>
                  </div>
                ) : (
                  <div className="flex items-center space-x-3 text-gray-500 bg-white/5 px-6 py-3 rounded-2xl border border-white/10">
                    <Clock size={20} />
                    <span className="text-xs font-black uppercase tracking-widest">Ainda Bloqueada</span>
                  </div>
                )}
              </div>
            </motion.div>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-3">
          {activeWorkouts.length > 0 ? activeWorkouts.map((t) => (
            <button 
              key={t.id}
            ...
            >
              ...
              <div className="w-full">
                <p className="font-black text-[11px] uppercase tracking-tighter line-clamp-1">{t.titulo}</p>
                <p className="text-[7px] text-gray-500 font-black uppercase mt-1">Ver Treino</p>
              </div>
              <ChevronRight size={12} className="absolute top-4 right-4 text-gray-800 group-hover:text-gold" />
            </button>
          )) : (
            <div className="col-span-full p-8 text-center bg-[#111] rounded-[2.5rem] border border-[#222]">
              <p className="text-gray-600 font-bold uppercase text-[10px] tracking-widest">Aguardando Fichas do Professor</p>
            </div>
          )}
        </div>
      </section>
      ...
      <div className="grid grid-cols-2 gap-4 pb-10">
        <div className="card-premium p-6 rounded-[2rem] text-center border-l-4 border-l-gold">
          <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest mb-1">Total Séries</p>
        ...
        >
          <p className="text-[10px] text-gold font-black uppercase tracking-widest mb-1">Evolução</p>
          <p className="text-xl font-black text-white flex items-center justify-center">Gráficos <ChevronRight size={16} /></p>
        </button>
      </div>
    </div>
  );
