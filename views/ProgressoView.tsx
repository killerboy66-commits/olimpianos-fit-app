...
import { ArrowLeft, Calendar, TrendingUp, Scale, Ruler, Plus, ChevronRight, X, Save, Percent, Layers, Zap, Sword, Shield, Trophy } from 'lucide-react';
...
const ProgressoView: React.FC<ProgressoViewProps> = ({ user, onBack }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setHistory(h.filter(p => p.user_id === user.id));
        setExercises(e);
        setAvaliacoes(a);
      } catch (error) {
  ...
  const handleSaveEval = async () => {
    ...
    try {
      ...
      notify("Avaliação física salva com sucesso! O professor já pode visualizar no Centro de Comando.", 'success');
      setShowAddModal(false);
      // Limpar campos
      setNewEval({
        peso: '', gordura: '', pescoco: '', ombros: '', torax: '', cintura: '', abdomem: '', quadril: '',
  ...
  const renderMedidaGridItem = (label: string, value?: number, suffix: string = "cm") => {
    ...
    return (
      <div className="bg-[#1A1A1A] p-3 rounded-2xl border border-[#222]">
        ...
        <p className="text-xs font-black text-white">{value}{suffix}</p>
      </div>
    );
  };

  const chartPerformanceData = history.slice(-7).map(h => ({
    name: new Date(h.date).toLocaleDateString('pt-BR', { weekday: 'short' }),
    peso: h.carga_kg || 0
  }));

  const chartFisicoData = avaliacoes.map(a => ({
  ...
  const firstEval = avaliacoes[0];

  const getRank = (workoutCount: number) => {
    if (workoutCount >= 50) return { title: 'Deus do Olimpo', color: 'text-gold', icon: <Zap size={10} /> };
    if (workoutCount >= 20) return { title: 'Herói Lendário', color: 'text-emerald-400', icon: <Trophy size={10} /> };
    if (workoutCount >= 5) return { title: 'Guerreiro de Elite', color: 'text-blue-400', icon: <Shield size={10} /> };
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-24">
      <header className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <button onClick={onBack} className="p-2 text-gold hover:bg-gold/10 rounded-full transition-all">
            <ArrowLeft size={24} />
          </button>
          <div>
            <h2 className="text-2xl font-black italic uppercase tracking-tighter">Evolução do Atleta</h2>
      ...
      <div className="flex p-1.5 bg-[#111] rounded-[1.5rem] border border-[#222]">
        <button 
          ...
          className={`flex-1 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'performance' ? 'bg-gold text-black shadow-xl' : 'text-gray-500'}`}
        >
          Performance Técnica
        </button>
        <button 
          onClick={() => setActiveTab('fisico')}
          className={`flex-1 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'fisico' ? 'bg-gold text-black shadow-xl' : 'text-gray-500'}`}
        >
          Composição Física
        </button>
      </div>
      ...
      {activeTab === 'performance' ? (
        <>
              ...
              {chartPerformanceData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                  <AreaChart data={chartPerformanceData}>
                    ...
                    <Tooltip contentStyle={{ backgroundColor: '#111', border: '1px solid #333', borderRadius: '16px', fontSize: '12px' }} />
                    <Area type="monotone" dataKey="peso" stroke="#C6A15B" strokeWidth={4} fillOpacity={1} fill="url(#colorPerf)" />
                  </AreaChart>
                </ResponsiveContainer>
              ) : (
          ...
          <section className="space-y-4">
            <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-4 flex items-center ml-2">
               <Calendar className="text-gold mr-2" size={14} /> Histórico de Séries
            </h3>
            {[...history].reverse().map((reg) => {
              const ex = exercises.find(e => e.id === reg.exercise_id);
              return (
                <div key={reg.id} className="card-premium p-6 rounded-[2.5rem] flex items-center justify-between border border-[#222] hover:border-gold/30">
                  <div className="flex items-center space-x-5">
                    ...
                    <div>
                      <p className="font-black text-white text-lg tracking-tight">{ex?.nome}</p>
                      <p className="text-[9px] text-gray-500 font-bold uppercase tracking-widest">{new Date(reg.date).toLocaleDateString()} • {new Date(reg.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-black text-gold italic">{reg.carga_kg}kg</p>
                    <p className="text-[9px] text-gray-600 font-black uppercase">{reg.reps_realizadas} Repetições</p>
                  </div>
                </div>
              );
            })}
        ...
        <div className="space-y-8 animate-in slide-in-from-right duration-300">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[#111] p-6 rounded-[2.5rem] border border-[#222] border-l-4 border-l-gold shadow-xl">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <p className="text-[9px] text-gray-500 font-black uppercase tracking-widest">Peso Atual</p>
              </div>
              <p className="text-3xl font-black text-white italic">{latestEval?.peso || '--'} <span className="text-xs text-gray-500 not-italic font-bold">KG</span></p>
            </div>
            <div className="bg-[#111] p-6 rounded-[2.5rem] border border-[#222] border-l-4 border-l-emerald-500 shadow-xl">
              <div className="flex items-center space-x-2 mb-2">
                <Percent size={14} className="text-emerald-500" />
                <p className="text-[9px] text-gray-500 font-black uppercase tracking-widest">Gordura Est.</p>
              </div>
              <p className="text-3xl font-black text-white italic">{latestEval?.gordura_percentual || '--'} <span className="text-xs text-gray-500 not-italic font-bold">%</span></p>
            </div>
          </div>
              ...
              {chartFisicoData.length > 1 ? (
                <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                  <LineChart data={chartFisicoData}>
                    ...
                    <Line type="monotone" dataKey="peso" stroke="#C6A15B" strokeWidth={5} dot={{ r: 6, fill: '#C6A15B', strokeWidth: 3, stroke: '#0A0A0A' }} activeDot={{ r: 8 }} />
                  </LineChart>
                </ResponsiveContainer>
              ) : (
                <div className="h-full flex items-center justify-center text-gray-600 italic text-center px-10 border-2 border-dashed border-[#222] rounded-3xl">
                   <p className="text-xs font-bold uppercase tracking-widest">Registre pelo menos duas avaliações para gerar o gráfico de tendência física.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <section className="space-y-6">
            <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-4 flex items-center ml-2">
               <Ruler className="text-gold mr-2" size={16} /> Relatório Antropométrico
            </h3>
            {avaliacoes.length > 0 ? (
               <div className="space-y-6">
                  {[...avaliacoes].reverse().map((evalItem) => (
                    <div key={evalItem.id} className="card-premium p-8 rounded-[3rem] border-2 border-[#222] relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-center mb-8 pb-4 border-b border-[#222]">
                        <div className="flex items-center space-x-3">
                           ...
                           <span className="text-sm font-black text-white uppercase italic tracking-tight">{new Date(evalItem.date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}</span>
                        </div>
              ...
              <div className="py-24 text-center border-2 border-dashed border-[#222] rounded-[3rem] bg-[#111]/30">
                ...
                <p className="text-gray-500 font-black uppercase text-xs tracking-[0.3em] italic">Aguardando Primeira Medição Oficial</p>
              </div>
            )}
          </section>
        </div>
      ...
      {/* Modal de Registro Premium Organizado */}
      {showAddModal && (
          ...
          <div className="bg-[#111] border border-[#333] p-8 rounded-[3rem] w-full max-w-2xl shadow-2xl animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col">
            <div className="flex justify-between items-center mb-8 shrink-0">
              <div>
                ...
                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Coleta de dados corporais completa</p>
              </div>
              <button onClick={() => setShowAddModal(false)} className="w-12 h-12 bg-[#1A1A1A] text-gray-400 hover:text-white rounded-full flex items-center justify-center transition-all border border-[#222]"><X size={24} /></button>
            </div>
            
            <div className="space-y-8 overflow-y-auto no-scrollbar pr-4 flex-1">
              {/* Seção 1: Métricas de Composição */}
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#1A1A1A] p-5 rounded-[2rem] border border-[#222] focus-within:border-gold transition-colors">
                  ...
                  <input type="number" placeholder="00.0" className="bg-transparent text-white font-black text-2xl outline-none w-full placeholder:text-gray-800" value={newEval.peso} onChange={e => setNewEval({...newEval, peso: e.target.value})} />
                </div>
                <div className="bg-[#1A1A1A] p-5 rounded-[2rem] border border-[#222] focus-within:border-gold transition-colors">
                  ...
                  <input type="number" placeholder="00" className="bg-transparent text-white font-black text-2xl outline-none w-full placeholder:text-gray-800" value={newEval.gordura} onChange={e => setNewEval({...newEval, gordura: e.target.value})} />
                </div>
              </div>

              {/* Seção 2: Tronco e Core */}
                  ...
                  {[
                    ...
                    <div key={f.id} className="bg-black/40 p-4 rounded-2xl border border-[#222] focus-within:border-gold/50 transition-colors">
                      ...
                      <div className="flex items-center">
                        <input type="number" step="0.1" className="bg-transparent text-white font-bold text-lg outline-none w-full" value={(newEval as any)[f.id]} onChange={e => setNewEval({...newEval, [f.id]: e.target.value})} />
              ...
              {/* Seção 3: Membros Superiores (Bilaterais) */}
              <div className="space-y-4">
              ...
              {/* Seção 4: Membros Inferiores (Bilaterais) */}
              <div className="space-y-4">
            ...
            <div className="flex gap-4 pt-8 shrink-0">
              <button onClick={() => setShowAddModal(false)} className="flex-1 py-5 text-gray-500 font-black uppercase text-xs tracking-widest hover:text-white transition-colors">Cancelar</button>
              <button 
