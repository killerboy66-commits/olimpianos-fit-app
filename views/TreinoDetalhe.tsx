...
import { ArrowLeft, CheckCircle2, Info, Youtube, Plus, Check, Timer, ChevronRight, Dumbbell, X, Sword, Shield, Zap, Flame } from 'lucide-react';
...
const TreinoDetalhe: React.FC<TreinoDetalheProps> = ({ user, initialWorkoutId, onBack }) => {
  ...
  // Cálculo do Mês Atual do Ciclo baseado na atribuição específica
  const getWorkoutMonth = (wId: string, currentAssignments: Atribuicao[]) => {
    ...
    return Math.min(Math.max(diffMonths, 0), 11) + 1; // 1 a 12
  };
  ...
  useEffect(() => {
    ...
    const loadWorkoutDetails = async () => {
      ...
      try {
        ...
        const currentItems = items
          .filter(i => i.user_id === user.id && i.workout_id === activeWorkoutId && (i.mes === viewMonth || (!i.mes && viewMonth === 1)))
        ...
        const loadPromises = currentItems.map(async (item) => {
          initialLoads[item.exercise_id] = await db.getExerciseLoad(user.id, activeWorkoutId, item.exercise_id);
        ...
        const todayProgress = allProgress.filter(p => 
          p.user_id === user.id && 
        ...
        currentItems.forEach(item => {
          completedMap[item.id] = todayProgress.filter(p => p.exercise_id === item.exercise_id).length;
        });
    ...
    loadWorkoutDetails();
  }, [activeWorkoutId, user.id, loading, assignments]);

  const activeWorkout = allWorkouts.find(t => t.id === activeWorkoutId);
  
  // O aluno agora só vê o mês atual do seu ciclo, sem opção de trocar manualmente
  const viewMonth = getWorkoutMonth(activeWorkoutId, assignments);
  
  // Captura todas as fichas ativas atribuídas a este aluno
  ...
  if (!activeWorkout) return <div className="p-10 text-center text-gray-500">Selecione uma ficha válida.</div>;
  ...
  return (
    <div className="pb-40 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      {/* Celebração de Exercício Concluído */}
      <AnimatePresence>
        {showCelebration && (
          <motion.div 
          ...
          >
            <div className="bg-gold text-black px-6 sm:px-10 py-4 sm:py-6 rounded-[2.5rem] sm:rounded-[3rem] shadow-2xl flex flex-col items-center border-4 border-white/20">
              ...
              <h3 className="text-2xl font-black uppercase italic tracking-tighter">Exercício Concluído!</h3>
              <p className="text-[10px] font-bold uppercase tracking-widest opacity-70">Digno de um Herói</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>

      {/* Header com Seletor Dinâmico de Fichas (ABCDE...) */}
      <div className="sticky top-0 bg-[#0A0A0A]/95 backdrop-blur-md pt-4 pb-6 z-20 border-b border-[#222] -mx-4 px-4">
        <header className="flex items-center justify-between mb-6">
          ...
          <div className="text-center flex-1">
            <h2 className="text-[10px] font-black text-gold uppercase tracking-[0.3em] mb-1">Ciclo de Treino Olimpiano</h2>
            <p className="text-white font-black text-sm uppercase">Ficha {activeWorkout.id}: {activeWorkout.titulo}</p>
          </div>
          <div className="w-10"></div>
        </header>

        {/* Indicador de Mês do Ciclo (Apenas leitura para o aluno) */}
        <div className="flex justify-center mb-6">
          <div className="bg-gold/10 border border-gold/20 px-6 py-2 rounded-2xl flex items-center space-x-3">
            <div className="w-2 h-2 bg-gold rounded-full animate-pulse"></div>
            <span className="text-[10px] font-black text-gold uppercase tracking-widest">Você está no Mês {viewMonth} do seu Ciclo</span>
          </div>
        </div>

        {/* Seletor de Todas as Fichas Disponíveis */}
        <div className="flex items-center space-x-3 overflow-x-auto no-scrollbar justify-center">
          {assignedWorkouts.map((w) => {
            ...
            const hasProgressToday = allProgress.some(p => 
              p.user_id === user.id && 
              p.workout_id === w.id && 
        ...
        {workoutItems.length > 0 ? (
          workoutItems.map((item) => {
            ...
            return (
              <div 
                key={item.id} 
                className={`card-premium p-6 rounded-[2.5rem] border-2 transition-all duration-300 ${isFullDone ? 'border-emerald-500/20 opacity-80' : 'border-[#222]'}`}
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-1">
                      <span className="bg-gold/10 text-gold text-[9px] font-black px-2 py-0.5 rounded uppercase">{ex.grupoNome}</span>
                    ...
                    {item.metodo && (
                      <div className="mt-1 flex items-center space-x-1.5">
                        ...
                        <span className="text-[10px] font-black text-gold uppercase tracking-widest">{item.metodo}</span>
                      </div>
                    )}
                  </div>
                  {ex.video_url !== '#' && (
                ...
                <div className="grid grid-cols-3 gap-2 mb-6">
                  <div className="bg-[#1A1A1A] p-3 rounded-2xl text-center border border-[#222]">
                    <p className="text-[8px] text-gray-500 font-black uppercase tracking-tighter mb-1">Séries</p>
                    <p className="text-sm font-black text-white">{doneSets}/{item.series}</p>
                  </div>
                  <div className="bg-[#1A1A1A] p-3 rounded-2xl text-center border border-[#222]">
                    <p className="text-[8px] text-gray-500 font-black uppercase tracking-tighter mb-1">Meta</p>
                    <p className="text-sm font-black text-white">{item.reps}</p>
                  </div>
                  <div className="bg-[#1A1A1A] p-3 rounded-2xl text-center border border-[#222]">
                    <p className="text-[8px] text-gray-500 font-black uppercase tracking-tighter mb-1">Pausa</p>
                ...
                {!isFullDone && (
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="bg-[#0F0F0F] border border-[#222] p-3 rounded-2xl">
                      <p className="text-[8px] text-gray-500 font-black uppercase text-center mb-1">Carga (KG)</p>
                      <input 
                        type="number" 
                    ...
                    <div className="bg-[#0F0F0F] border border-[#222] p-3 rounded-2xl">
                      <p className="text-[8px] text-gray-500 font-black uppercase text-center mb-1">Repetições</p>
                      <input 
                        type="number" 
                        className="bg-transparent text-white font-black text-2xl text-center outline-none w-full"
                        value={exerciseReps[ex.id] || ''}
                        onChange={(e) => setExerciseReps(prev => ({ ...prev, [ex.id]: e.target.value }))}
                      />
                    </div>
                  </div>
                )}
                ...
                {exerciseProgress.length > 0 && (
                  <div className="mb-6 flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
                    {exerciseProgress.map((p, idx) => (
                      <div key={p.id} className="bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-xl">
                        <span className="text-[9px] font-black text-emerald-400">S{idx+1}: {p.carga_kg}kg × {p.reps_realizadas}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div className="flex items-center gap-3">
                  <button className="p-4 bg-[#1A1A1A] rounded-2xl text-gray-500 border border-[#222]" onClick={() => notify(ex.descricao, 'info')}>
                    <Info size={20} />
                  </button>
                  <button 
                    ...
                    className={`flex-1 py-4 rounded-2xl font-black flex items-center justify-center space-x-2 transition-all ${
                      isFullDone ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-gold text-black'
                    }`}
                  >
                    {isFullDone ? <Check size={20} strokeWidth={3} /> : <Plus size={20} strokeWidth={3} />}
                    <span className="uppercase tracking-widest text-xs">{isFullDone ? 'Ficha Concluída' : `Registrar Série ${doneSets + 1}`}</span>
                  </button>
                </div>
              </div>
            );
          ...
          <div className="p-20 text-center bg-[#111] rounded-[2.5rem] border border-[#222]">
            ...
            <p className="text-gray-500 font-bold">Nenhum treino programado pelo professor para o Mês {viewMonth} da Ficha {activeWorkout.id}.</p>
            <p className="text-[10px] text-gray-600 mt-2 uppercase font-black">Tente selecionar outro mês acima</p>
          </div>
        )}
      </div>

      {/* Cronômetro Flutuante de Recuperação */}
      <AnimatePresence>
        {timerActive && (
          <motion.div 
          ...
          >
            <div className="bg-[#111] border-2 border-gold rounded-[2.5rem] p-6 shadow-2xl backdrop-blur-xl relative overflow-hidden">
              ...
              <div className="absolute top-0 left-0 h-1 bg-gold transition-all duration-1000" style={{ width: `${(timeLeft / initialTime) * 100}%` }}></div>
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <div className={`p-4 rounded-2xl ${timeLeft <= 10 ? 'bg-red-500 text-white animate-pulse' : 'bg-gold text-black'}`}>
                    <Timer size={28} />
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Descanso Ativo</p>
                    <p className={`text-4xl font-black tabular-nums ${timeLeft <= 10 ? 'text-red-500' : 'text-white'}`}>{formatTime(timeLeft)}</p>
                  </div>
                </div>
                <div className="flex flex-col space-y-2">
                  <button onClick={() => setTimeLeft(prev => prev + 30)} className="p-3 bg-[#1A1A1A] text-white rounded-xl border border-[#333] text-[10px] font-black uppercase">+30s</button>
                  <button onClick={() => setTimeLeft(0)} className="px-4 py-2 bg-white text-black rounded-xl font-black uppercase text-[10px]">Pular</button>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <div className="mt-12 mb-10 text-center">
        <button onClick={onBack} className="px-12 py-5 bg-[#111] border border-[#222] rounded-full text-gray-500 font-black text-[10px] uppercase tracking-[0.3em] hover:text-white transition-colors">Finalizar Treino</button>
      </div>
    </div>
  );
};
